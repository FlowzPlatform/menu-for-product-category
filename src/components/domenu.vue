...
<script>
  ...
  let slug = ''
  let item = ''
  let authorization_token = "xxxxxxx"
  ...
  export default {
    ...
    data: () => ({
      ...
      methods: {
        ...
        save_category(){
         ...
         notice.config({
          ...
          axios({
                method:'post',
                url:"http://localhost:8181/categories/*/sites/" + this.$route.params.site_id,
      ...
      let results = JSON.parse($('#domenu-0').domenu().toJson())
      // console.log(results,"+++++++++++++++")
      let resultObj = Object.assign({"category_list": results,"site_id": this.$route.params.site_id,"id":id});
      console.log(resultObj)
      axios({
            method:'post',
            url:"http://localhost:8181/categories/*/sites/" + this.$route.params.site_id +"/createCategory",
            headers: {'Authorization': authorization_token,"content-type": "application/json"},
            data : resultObj
          }).then(response => {
            // console.log("************",response);
   ...
   async postAllSlugs(slugAndHref){
    //  console.log("called.......................")
     axios({
           method:'get',
           url:"http://localhost:8181/categories/*/sites/" + this.$route.params.site_id + "/slugs/*",
           headers: {'Authorization': authorization_token}
         }).then(await(response => {
              // console.log("^^^^^^^^^^^^^^^",response)
              ...
              let slugHrefObj = Object.assign({slugAndHref,"site_id": this.$route.params.site_id,"id":slugId});
    ...
    mounted() {
      ...
      axios({
        ...
        //   $(document).ready(function() {
        //   $('#popover-content').find("#href").on('keyup', function(){
        //     console.log("HREF",mee.href)
        //     mee.validateHref(mee.href)
        //   })
        // })
}
}
